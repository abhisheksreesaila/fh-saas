...
# %% auto 0
__all__ = ['get_or_create_tenant_db', 'TenantUser', 'TenantPermission', 'TenantSettings', 'WebhookEvent', 'WebhookSecret',
...
class WebhookEvent:
    ...
    webhook_id: str      # Primary key (UUID hex)
    source: str          # Webhook source (e.g., 'stripe', 'github')
    event_type: str      # Event type (e.g., 'payment.succeeded')
    payload_json: str    # Full webhook payload as JSON
    signature: str       # Request signature for verification
    idempotency_key: str # Unique key to prevent duplicate processing (UNIQUE)
    status: str = 'pending'  # 'pending', 'completed', 'failed'
    error_message: str = None  # Error details if failed
    created_at: str      # When webhook was received
    processed_at: str = None  # When webhook was processed

class WebhookSecret:
    """Per-source webhook secrets for signature verification"""
    source: str          # Primary key (e.g., 'stripe', 'github')
    secret_key: str      # Secret for HMAC verification
    is_active: bool = True
    created_at: str
...
def init_tenant_core_schema(tenant_db: Database):
    ...
    webhook_events = tenant_db.create(WebhookEvent, name="webhook_events", pk='webhook_id')
    webhook_secrets = tenant_db.create(WebhookSecret, name="webhook_secrets", pk='source')
    
    # Create unique index on idempotency_key for webhook events
    try:
        tenant_db.execute(
